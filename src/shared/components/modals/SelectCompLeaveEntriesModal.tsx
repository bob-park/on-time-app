...
export default function SelectCompLeaveEntriesModal({
  ...
  return (
    <Modal visible={show} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View className="mt-6 flex size-full flex-col items-center gap-3">
        {/* headers */}
        <View className="relative w-full">
          ...
          <TouchableOpacity
            className="absolute left-4 top-0 size-10 items-center justify-center rounded-full bg-gray-100"
            onPress={() => onClose()}
          >
          ...
          <TouchableOpacity
            className="absolute right-4 top-0 h-10 w-20 items-center justify-center rounded-xl bg-gray-100"
            onPress={handleSelect}
          >
            <Text className="font-bold">선택</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="size-full p-4">
          <FlashList
            ...
            ListHeaderComponent={
              <View className="mb-1 mt-1 flex w-full flex-row items-center gap-2 rounded-t-xl bg-gray-100 p-3">
                <Text className="w-44 flex-none text-center text-sm font-bold" ellipsizeMode="tail">
                  내용
                </Text>
                <Text className="w-24 flex-none text-center text-sm font-bold">발생일</Text>
                <Text className="w-12 flex-none text-center text-sm font-bold">휴가일</Text>
                <Text className="w-12 flex-none text-center text-sm font-bold">잔여일</Text>
              </View>
            }
    ...
    <TouchableOpacity className="my-2" onPress={handleToggle}>
      <View
        className={cx('flex h-20 w-full flex-row items-center gap-2 rounded-2xl px-3 py-2', {
          'border-2 border-gray-200 bg-gray-200': selected,
        })}
      >
        <Text className="w-44 flex-none text-sm" ellipsizeMode="tail" numberOfLines={2}>
          {entry.contents}
        </Text>
        <Text className="w-24 flex-none text-sm">{dayjs(entry.effectedDate).format('YYYY-MM-DD')}</Text>
        <Text className="w-12 flex-none text-right text-sm font-semibold">{entry.leaveDays}</Text>
        <Text className="w-12 flex-none text-right text-sm font-semibold">{entry.usedDays}</Text>
      </View>
    </TouchableOpacity>
  );
};
